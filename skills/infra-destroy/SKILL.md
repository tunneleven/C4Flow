...
`terraform destroy` calls the Cloudflare provider to remove the DNS record — it needs `CLOUDFLARE_API_TOKEN`. Check before the user commits to the destructive confirmation.
...
if [ -n "$CLOUDFLARE_API_TOKEN" ]; then
  ...
  echo "  echo 'export CLOUDFLARE_API_TOKEN=your-token-here' > ~/.cloudflare"
